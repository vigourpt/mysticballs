...
DROP FUNCTION IF EXISTS public.increment_reading_count(uuid);
...
-- Create function to get average readings before signup
CREATE OR REPLACE FUNCTION public.get_avg_readings_before_signup()
RETURNS FLOAT AS $$
DECLARE
  v_avg_readings FLOAT;
BEGIN
  SELECT AVG(previous_readings_count)::FLOAT INTO v_avg_readings
